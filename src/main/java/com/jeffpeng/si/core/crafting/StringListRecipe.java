...
public class StringListRecipe implements IRecipe {
	...
	private void processResult(String result){
		Object is = SILib.stringToItemStack(result);
		if(is instanceof ItemStack){
			this.result = (ItemStack)is;
			valid = true;
		} else if(is instanceof String){
			ItemStack firstEntry = SILib.getFirstOreDictMatch((String) is);
			if(firstEntry != null){
		...
		if(valid == false) FMLLog.warning("Cannot resolve " + result); else FMLLog.info("Registered StringListRecipe for " + result);
	...
	public StringListRecipe(String result, List<String[]> rawShape){
	...
	private boolean matchAgainstOffset(int w, int h,InventoryCrafting inv,boolean mirrorx, boolean mirrory){
		FMLLog.info("coords " + w + " " + h + ((mirrorx) ? "x" : "") + ((mirrory) ? "y" : ""));
		...
		for(int x = 0; x < inv.inventoryWidth; x++){
			for(int y = 0; y < invheight; y++){
				...
				FMLLog.info("XY " + x + y + " TXY " + tx + " " + ty);
				
				if(ty < 0 || tx < 0 || tx >= this.width || ty >= this.height){
					FMLLog.info("ca0");
					if(inv.getStackInRowAndColumn(x, y) != null) return false;
					FMLLog.info("approved");
				} else {
					FMLLog.info("cas");
					if(!SILib.matchItemStacksOreDict(inv.getStackInRowAndColumn(x, y), lines.get(ty)[tx])) return false;
					FMLLog.info("approved");
				}
	...
	public Object[] getIngredientArray(){
		...
		for(int y = 0; y < this.height;y++){
			for(int x = 0; x < this.width;x++){
				Object ingredient = SILib.stringToItemStack(lines.get(y)[x]);
