...
public class AddCentrifugeRecipe extends BasicAction {
	...
	private static void initReflection(){
		//TODO: This whole atrocity is only here because of a bug with the Centrifuge API in v10x, remove when v12 hits and works
	...
	public boolean on(FMLLoadCompleteEvent event){
		...
		try{
			...
			log.info("onlce addCentrifuge outstacks done");
			
			outputitemsarray = outputitems.toArray(new Object[outputitems.size()]);
			
			log.info("onlce addCentrifuge outarray done");
	...
	@Override
	public void execute(){
		log.info("execute addCentrifuge");
		try {
			for(ItemStack ins : in){
				if(this.outFluid == null){
					if(ins!=null) addCentrifugeRecipe.invoke(RecipesCentrifuge.getRecipes(), ins, null, RecipeLevel.API, outputitemsarray);
				} else {
					if(ins!=null) addCentrifugeRecipe.invoke(RecipesCentrifuge.getRecipes(), ins, FluidOutConstructor.newInstance(this.outFluid,this.outputfluidChance), RecipeLevel.API, outputitemsarray);
				}
			}
		} catch(Exception e){
			log.info("#aa#");
